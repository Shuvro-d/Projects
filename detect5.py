...
def run(
    ...
    screenshot = source1.lower().startswith('screen')
    ...
    if webcam:
        ...
        dataset = LoadStreams(source1, img_size=imgsz, stride=stride, auto=pt, vid_stride=vid_stride)
        bs = len(dataset)
    elif screenshot:
        dataset = LoadScreenshots(source1, img_size=imgsz, stride=stride, auto=pt)
    else:
        dataset = LoadImages(source1, img_size=imgsz, stride=stride, auto=pt, vid_stride=vid_stride)
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            IN1=IN1+1
            if len(det)>=1:# getting the true positive
                DN1=DN1+1
            if len(det):
...
##########################################################second portion###########################################################
    ...
    webcam = source2.isnumeric() or source2.endswith('.txt') or (is_url2 and not is_file2)
...
###########################################################printing results and confusion matrix###################################
    print("*******Results*******")
    if DN1>(IN1-DN1):
        TP=DN1
        FN=IN1-DN1
    ...
    print(f"False Negetive ={FN}")
    print(f"True Negetive = {TN}")
    print(f"False Positive ={FP}")
    precision=(TP/(TP+FP))*100
    recall=(TP/(TP+FN))*100
    F1=((2*(TP/(TP+FP))*(TP/(TP+FN)))/((TP/(TP+FP))+(TP/(TP+FN))))*100
    ...
    cfm = [[TP, FN],[FP, TN]]
    df_cfm = pd.DataFrame(cfm, index = classes, columns = classes,dtype=np.int32)
    plt.figure(figsize = (10,7))
    cfm_plot = sn.heatmap(df_cfm, annot=True,cmap="Blues",fmt='g')
    cfm_plot.figure.savefig("/content/Confusion matrix.png")
    print("saved the confusion matrix")
